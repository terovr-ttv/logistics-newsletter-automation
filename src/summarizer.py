...
class NewsletterItem(BaseModel):
    ...
    body: str = Field(
        description=(
            "2 to 3 sentences of substance, written in the Between the Lines newsletter voice. "
...
class Newsletter(BaseModel):
    ...
    opening: str = Field(
        description=(
            "One paragraph of about 80 words. Open with energy. Ideally lead with a real quote "
            "or sharp stat pulled from the source articles. Name the single most important thing "
            "this month and preview what's inside the issue. Include inline [Source] citations. "
            "No line breaks."
        )
    )
    big_story_headline: str = Field(
        description=(
            ...
            "sentence, sentence case. Example: 'The freight market has turned, and this time "
            "it looks structural rather than seasonal.'"
        )
    )
    big_story_paragraphs: list[str] = Field(
        description=(
            "3 to 5 paragraphs, about 300 words total combined, on the deepest story of the "
            "month. Each element is one paragraph of 2 to 4 sentences. Include [Source] "
            "citations inline. No line breaks within any paragraph."
        )
    )
    quick_takes: list[NewsletterItem] = Field(
        description=(
            "4 to 6 shorter items covering other notable developments worth knowing. "
            "Each item body is 2 to 3 sentences with inline [Source] citations."
        )
    )
    what_to_watch: list[NewsletterItem] = Field(
        description=(
            "2 to 3 forward-looking items about emerging stories that will shape next month. "
            "Brief and specific."
        )
    )
    bottom_line: str = Field(
        description=(
            "A single closing paragraph of about 60 words. What does this month tell carriers "
            "and final-mile operators about where the industry is heading? Confident, "
            "partner-tone observation. Not a sales pitch. No line breaks."
        )
    )
...
(for P&D fleets) and the Truckload Driver Finisher Program (for line-haul carriers). Our \
readers are fleet executives, safety directors, and operations managers at trucking and \
final-mile companies. They read this newsletter to stay current on the freight and logistics \
market without wading through a dozen trade publications.

VOICE
- Write like a human industry insider who's read all this so the reader doesn't have to.
- Conversational and professional. Contractions are welcome. Talk with the reader, not at them.
- Open sections with a vivid framing line, a sharp number, or a real quote pulled from the \
source articles. Never open with "This month..." or "In recent news..."
...
This is the single strongest AI tell in prose. When you would reach for one, use a comma, \
period, colon, semicolon, or parentheses instead. Rewrite the sentence if you have to.
...
TARGET LENGTH
About 1,000 to 1,300 words across all fields combined. Fits on two 8.5x11 printed pages.
